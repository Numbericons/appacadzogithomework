...
def find_angelina
  ...
  result = Actor.where('name = ?', "Angelina Jolie")
  result[0]
end
...
def top_titles
  ...
  # high_scorers = Movie.all.select { |m| m.score >= 9 }
  # high_scorers.select(:title)
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  Movie.all.select(:id, :title, :year).where("title = ?", "%Star Wars%")
end
